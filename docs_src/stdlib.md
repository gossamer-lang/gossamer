...
This is the Gossamer 0.64.0 standard library reference.
...
| [`std::encoding::json`](#stdencodingjson) | 22 | JSON parser, emitter, and derive support. |
...
| [`std::sync`](#stdsync) | 18 | Synchronisation primitives beyond channels. |
...
| [`std::time`](#stdtime) | 26 | Wall-clock and monotonic time facilities. |
...
| `as_u64` | fn | Reads a dynamic Value as Option<u64>, for a non-negative integer up to u64::MAX. |
...
| `File` | type | Streaming file handle. Reads and writes at the handle's own cursor (read, read_to_string, write, write_bytes, seek), positionally (read_at, read_at_into, write_at), and reports size (len, set_len). Durability is sync_all / sync_data; multi-process safety is the try_lock_* / unlock family. |
...
| `accept` | fn | Validate a WebSocket upgrade request and answer the 101 Switching Protocols response that completes the handshake. |
...
| `find` | fn | Returns the character index of the first match, or None. |
...
| `rfind` | fn | Returns the character index of the last match, or None. |
...
| `Mutex` | type | Mutual-exclusion lock: `lock()` / `unlock()` around the code it guards. |
| `Once` | type | One-shot initialisation latch. |
| `Receiver` | type | The receiving end of a channel: `recv() -> Option<T>`, `None` once it is closed and drained. |
| `RwLock` | type | Reader-writer lock guarding an `i64`: `read`, `write`, `with_read`, `with_write`. |
| `Sender` | type | The sending end of a channel: `send(v)`, `close()`. |
...
| `Duration` | type | A span of nanoseconds: `from_nanos` / `from_micros` / `from_millis` / `from_secs` / `from_secs_f64`, the matching `as_*` accessors, `+`, `-`, and ordering. |
| `Instant` | type | A monotonic clock reading in nanoseconds: `now()`, `elapsed()`, `elapsed_ms()`, `duration_since(earlier)`. |
...
| `sleep` | fn | Suspends the current goroutine for a `Duration`, or for an integer count of milliseconds. |
...
| `__sleep_ns` | builtin | Internal: suspends the current goroutine for a nanosecond count; the lowering of `sleep` and `sleep_ctx`. |
| `__sleep_ns_ctx` | builtin | Internal: context-aware nanosecond sleep; the lowering of `sleep_ctx` that wakes early when the context fires. |
