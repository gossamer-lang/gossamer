...
/// Registers every `gos_rt_*` runtime helper against the JIT builder so
/// compiled bodies can call into the runtime in-process.
///
/// The names and addresses come from the table the runtime generates from
/// its own definitions, so every helper the runtime defines resolves here
