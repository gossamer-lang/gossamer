...
- A goroutine parked on a socket or a sleep is woken by the socket's own readiness word or the poller's timer heap, where every wait took a process-wide lock, two registration syscalls, and a heap-allocated waker; `time::sleep` inside a handler no longer queues behind other connections' waits.
