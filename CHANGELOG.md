...
- `json::encode` of a struct escapes `<`, `>`, `&`, U+2028, and U+2029 in its strings on the compiled tiers and the JIT, as the bytecode VM always has, where those builds wrote them as they were.
- Appending a scalar to a `String` - `s.push_str(n.to_string())`, `s += n.to_string()`, `s = s + n.to_string()`, or a `format` piece - writes the scalar's text straight into the receiver for every integer width, both float widths, `bool`, and `char`, where every form but an `i64` or `f64` piece built a `String` for the text first and then copied it.
- A release build appends ASCII text to a `String` it holds alone without a runtime call: `s.push_str("...")`, `s += "..."`, and `s.push_str(t)` for an all-ASCII `t` copy the bytes and set the length in place, and every other shape keeps the runtime path.
- HTTP/1.1 and HTTPS connections on the compiled tiers are served on goroutines over a readiness poller instead of an OS thread each: a connection waiting for its next request holds no thread and no allocator heap of its own, so a server with thousands of keep-alive clients holds a fraction of the memory it did, and throughput holds up as connections grow rather than falling off.
- A goroutine parked on a socket or a sleep is woken by the socket's own readiness word or its worker's timer heap, where every wait took a process-wide lock, two registration syscalls, and a heap-allocated waker; `time::sleep` inside a handler no longer queues behind other connections' waits.
- Each scheduler worker reads socket readiness and timers for its own goroutines: a worker with nothing to run waits in its own poller, so a request that arrives wakes the thread that serves it directly instead of passing through a poller thread and a second wakeup.
- A new goroutine starts on the worker running the fewest goroutines. A goroutine stays on the worker it starts on, and new ones used to go to whichever worker checked the shared queue first, so a server's connections could all land on one thread while the others sat idle.
- The scheduler's watchdog no longer signals a worker that is parked with nothing to run, and no longer asks every running goroutine to yield because some worker has been idle; an idle server took about ten wakeup signals a second per worker, and a busy one yielded early every few milliseconds.
- The runtime's shared I/O poller thread starts the first time something registers with it and sleeps until its next timer or registration, where every program started it and woke it every millisecond.
- A thread about to wait indefinitely - a server's accept loop, a worker with nothing to run - hands the allocator pages it freed back first, and the scheduler's watchdog returns memory to the operating system once its purge delay has passed, so a process that allocated a large working set at startup and then went quiet no longer keeps it resident.
- `yaml::to_json` and `yaml::parse` decode straight into the JSON value without building an intermediate YAML tree, so converting a large document holds one tree, not two; the compiled tiers now apply the same size and nesting caps as the bytecode VM.
- On the bytecode VM, the document `yaml::parse` answers is a `json::Value` that `json::get`, `json::keys`, and every other `json::` accessor read, as on the compiled tiers; the accessors answered `None` for it before.
- Calling a method that `json::Value` or `DynValue` does not declare is reported by `gos check` (GT0002) at the call, where the VM ignored it and a native build failed on an undefined symbol.
- HTTP responses on HTTP/1.1 omit `connection: keep-alive`, which the protocol already implies, and send `connection: close` only when the connection is ending. An HTTP/1.0 request that asks for keep-alive gets it on the bytecode VM too, and a `Connection` header is read as a list of tokens there, as on the compiled tiers.
- A scheduler worker with work queued for it no longer sleeps out its idle timeout: a wake that arrived just before the worker went to sleep left the task waiting up to 50 ms, which showed as a long tail on requests at any concurrency.
- Every TCP listener - `http::serve`, `http::Server`, `serve_tls`, WebSockets, `net::TcpListener::bind`, the HTTP/2 and metrics servers - accepts with the system's full connection queue rather than 128, so a burst of clients connecting at once is queued instead of dropped into a one-second retransmit.
- A handler given to `http::serve`, `http::Server::serve`, `serve_tls`, or a WebSocket server has everything it captured counted atomically, as a router's routes already did; connections served concurrently no longer race the reference counts of a closure's or a handler struct's strings.
- Every program commits allocator memory as it is used, on every platform, so an OS thread no longer keeps whole allocator pages resident for every size it touched where the allocator used to commit its arenas up front (glibc builds among them: `gos run`, the JIT, `gos build --dynamic`). `MIMALLOC_ARENA_EAGER_COMMIT` still overrides it.
- On the bytecode VM, `fs::read` answers packed bytes, one per byte of the file, where every byte was a boxed integer.
- On the bytecode VM, a write through a place several steps deep (`st.tables[t].slots[i] = v`, `st.tables[t].slots.resize(n, 0)`) writes where the value lives, and a read chain releases the levels it passed through, so growing a Vec inside a struct inside a Vec costs the rows added rather than a copy of the Vec per row. A function answering nothing runs its closing statement the same way.
- A release binary copies and fills memory through routines sized for short moves, where every copy started with the microcode sequence musl's `memcpy` uses for long ones; string building, struct moves, and map keys all take the shorter path.
