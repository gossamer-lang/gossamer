...
`project.toml` manifest. Most documented
...
- **Prefer dedicated collection contracts for intent.** `Stack` is the
  ...
  are rejected (GR0006).
- **`Map` and `BTreeMap` are distinct types**: a `Map` hashes, a `BTreeMap`
  keeps its keys in order in a B+ tree (`get`, `insert`, `remove` in
  O(log n), a walk in key order with no sort). Each constructor answers its
  own, and neither converts to the other. Only the ordered pair answers
  `m.first_key_value()`, `m.last_key_value()`, `m.pop_first()`,
  `m.pop_last()` (each an `Option<(K, V)>`) and `m.range(lo..hi)` - any range
  form written in the call, its bounds keys (`m.range("b".."f")`,
  `m.range(lo..=hi)`, `m.range(lo..)`); a range kept in a binding reports
  GT0091. `BTreeSet` answers `first`, `last`, `pop_first`, `pop_last`, and
  `range` the same way. A key or element type that writes its own `cmp`
  (`impl Ord for Ranked`) is ordered by that body on every tier; every other
  type takes the language's order.
...
- **Append text out of a byte buffer in place**: `s.push_utf8(buf, a, b)`
  appends the `[a, b)` window when it is valid UTF-8, and
  `s.push_json_quoted(buf, a, b)` appends it as a quoted JSON string
  escaped as `json::encode` escapes one. Both answer whether they appended,
  so a renderer never copies the window into a `String` of its own first.
...
- **Fixed set of compiler-known calls, each written as an ordinary
  ...
  `{:o}`, precision `{:.2}`; no positional `{0}`, which is GP0021 - bind the
  value and name it); `matches`, `todo`, `unimplemented`,
  `unreachable`, `dbg`; build-time `regex::compile` / `sql::statement`
  (validate the literal, and the older `regex!` / `sql!` spellings
  report GP0051) and `codegen` (splice the code a `comptime fn`'s String
  spells, typed as that code is where it lands).
...
- **Semicolons are same-line separators only**: `let a = 1; let b = 2`
  ...
  a newline or `}`, is invalid. A newline followed by leading `&`, `*`, `-`,
  or `|` starts a NEW statement (so a closure `|x| x + k` can end a function
  on its own line); for multi-line continuation, end the previous line with
  the operator or parenthesize. A leading `||` still continues a condition.
...
- **Types**: `bool char i8..i64 u8..u64 isize usize f32 f64 String
  ...
  / `u128` are rejected (GT0014). Each numeric primitive carries its limits
  (`i64::MIN`, `u8::MAX`, `u32::BITS`, `f64::EPSILON`, `f64::NAN`,
  `f64::INFINITY`, `f64::MIN_POSITIVE`). Transparent `type Id = i64` /
...
The depth differs by tier because the limit does: the bytecode VM refuses a
call at 4096 frames and prints its call stack with repeated frames collapsed,
while JIT-compiled and native frames run until the installed stack guard trips
on the real OS stack. Neither corrupts memory. On the VM a call in tail
position - the last thing a function body, an `if` arm, or a `match` arm
does - replaces its caller's frame, so tail recursion runs at any depth there.
...
- `[T; N]` is an owned fixed array, `[T]` is an unsized borrowed slice,
  ...
  support `sort`, `reverse`, `swap`, and `fill` without resizing. Order by a
  key with `xs.sort_by_key(|p| p.score)` (`Reverse(k)` for descending); a
  `sort_by` comparator answers a sign, and a scalar has no `cmp` (it orders
  with `<`), so compare fields: `|a, b| if a.x < b.x { -1 } else if a.x > b.x
  { 1 } else { 0 }`. `%i`
...
- `std::collections`: `Vec`, `Map` (any hashable key by value;
  ...
  every traversal) / `BTreeSet` (the sorted set), `BTreeMap` (sorted by key,
  any key the language orders - integers of every width including `u64`,
  floats in total order, `bool`, `char`, `String`, tuples, structs, enums;
  ordered ranges and ends as above), `Deque`, `Queue`, `Stack` (each holds whatever a `Vec<T>` holds),
...
- Core: `fmt io env fs path os process time context flag errors
  ...
  RFC 3339 parse/format. `Instant` and `Duration` count nanoseconds and
  are their own types, never an integer: `t.elapsed()`,
  `later.duration_since(t)`, `d.as_millis()` / `as_nanos()` /
  `as_secs_f64()`, `Duration::from_micros(n)`, `d + e`, `d < e`. `flag`: `Set::new` + `string/int/bool`
...
- Misc: `sort`, `math::{rand, big}`, `crypto::{rand, sha256, sha512,
  ...
  `hash::{fnv, crc32, crc32c, adler32}`, `uuid` (v4/v7), `jwt`,
...
Bindings are aimed at `gos build`, supported by `gos run` / `gos test`,
and not loaded by the REPL.
